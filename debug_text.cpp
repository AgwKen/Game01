...
namespace hal
{
	...
	{
		// Ensure there is at least one line before accessing back()
		if (m_TextLines.empty()) {
			m_TextLines.emplace_back();
		}

		m_TextLines.back().strings.emplace_back(color); // Add new string with color

		for (; *pText; pText++) {

			if (*pText == '\n') {
				m_TextLines.emplace_back(); // Add new line
				m_TextLines.back().strings.emplace_back(color); // Optionally add new string with color
			}
			else if (*pText == '\r') {
				...
				while (m_TextLines.back().characterCount % 4 != 0) {
					...
					m_TextLines.back().strings.back().characters += ' '; // Add space for tab
