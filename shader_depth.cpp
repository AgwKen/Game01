...
bool ShaderDepth_Initialize()
{
    ...
    if (FAILED(hr)) {
        hal::dout << "shader_depth_Initialize() シェーダーの読み込みに失敗しました" << std::endl;
        return false;
    }
    ...
    if (!ifs_ps) {
        MessageBox(nullptr, "ピクセルシェーダーの読み込みに失敗しました\n\nshader_pixel_depth.cso", "Error", MB_OK);
        return false;
    }
    ...
    if (FAILED(hr)) {
        hal::dout << "ShaderDepth_Initialize() シェーダーの読み込みに失敗しました" << std::endl;
        return false;
    }
